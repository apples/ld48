...
func _on_TileMapObstacles_cell_changed(tilemap, pos, from_tile, to_tile, flip_x, flip_y, transpose, autotile_coord):
	if sync_trunks:
		...
		if to_tile == TileType.BIGTREETRUNK:
			match get_cellv(toppos):
				TileType.NONE:
					set_cellv(toppos, TileType.BIGTREECANOPY, flip_x, flip_y, transpose)
					update_bitmask_area(toppos)
		elif from_tile == TileType.BIGTREETRUNK:
			match get_cellv(toppos):
				TileType.BIGTREECANOPY:
					set_cellv(toppos, TileType.NONE, flip_x, flip_y, transpose)
					update_bitmask_area(toppos)
